...
func TestRawParser(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			wantTitleZh: "BanG Dream! 少女乐团派对！☆PICO FEVER！",
			wantRes:    "1080p",
			wantEp:     26,
			wantSeason: 1,
			wantSub:    "简繁",
		},
		{
			...
			wantGroup:  "喵萌奶茶屋",
			wantTitleEn: "Summer Time Rendering",
			wantRes:    "1080p",
			wantEp:     11,
			wantSeason: 1,
			wantSub:    "繁日",
		},
		{
			...
			wantTitleZh: "关于我在无意间被隔壁的天使变成废柴这件事",
			wantTitleEn: "Otonari no Tenshi-sama",
			wantRes:    "1080p",
			wantEp:     9,
			wantSeason: 1,
			wantSub:    "繁",
		...
		},
		{
			...
			wantGroup:  "织梦字幕组",
			wantTitleZh: "尼尔：机械纪元",
			wantTitleEn: "NieR Automata Ver1.1a",
			wantRes:    "1080P",
			wantEp:     2,
			wantSeason: 1,
			wantSub:    "简日",
		},
		{
			...
			wantGroup:  "MagicStar",
			wantTitleZh: "假面骑士Geats",
			wantTitleJp: "仮面ライダーギーツ",
			wantRes:    "1080p",
			wantEp:     33,
			wantSeason: 1,
		},
		...
		},
		{
			...
			wantGroup:  "喵萌奶茶屋",
			wantTitleZh: "银砂糖师与黑妖精",
			wantTitleEn: "~ Sugar Apple Fairy Tale ~",
			wantRes:    "1080p",
			wantEp:     13,
			wantSeason: 1,
			wantSub:    "简日",
		},
		{
			...
			wantGroup:  "ANi",
			wantTitleZh: "16bit 的感动 ANOTHER LAYER",
			wantRes:    "1080P",
			wantEp:     1,
			wantSeason: 1,
			wantSub:    "繁",
		},
		{
			...
			wantTitleZh: "终末列车寻往何方",
			wantTitleEn: "Shuumatsu Torein Dokoe Iku",
			wantEp:     12,
			wantSeason: 1,
		},
		{
		...
		},
		{
			...
			wantGroup:  "LoliHouse",
			wantTitleZh: "2.5次元的诱惑",
			wantTitleEn: "2.5-jigen no Ririsa",
			wantRes:    "1080p",
			wantEp:     1,
			wantSub:    "简繁",
		},
		...
		},
		{
			...
			wantGroup:  "ANi",
			wantTitleEn: "Grand Blue Dreaming",
			wantTitleZh: "GRAND BLUE 碧蓝之海",
			wantSeason: 2,
			wantEp:     4,
			wantSub:    "繁",
		},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.wantTitleZh != "" && info.TitleZh != tt.wantTitleZh {
				t.Errorf("TitleZh = %v, want %v", info.TitleZh, tt.wantTitleZh)
			}
			if tt.wantTitleEn != "" && info.TitleEn != tt.wantTitleEn {
				t.Errorf("TitleEn = %v, want %v", info.TitleEn, tt.wantTitleEn)
			}
			if tt.wantTitleJp != "" && info.TitleJp != tt.wantTitleJp {
				t.Errorf("TitleJp = %v, want %v", info.TitleJp, tt.wantTitleJp)
			}
