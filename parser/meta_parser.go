...
func (p *TitleMetaParser) findallSubTitle(pattern *regexp2.Regexp, sym string) [][]string {
	...
	for match != nil {
		...
		// for i := 1; i < len(match.Groups()); i++ {
		// 	groups = append(groups, match.Groups()[i].String())
		// }
	...
	// 如果找到匹配，执行替换
	if len(ans) > 0 {
		p.title, _ = pattern.Replace(p.title, sym, -1, -1)
	}
...
func (p *TitleMetaParser) nameProcess() (string, string, string) {
	...
	if len(p.token) == 1 {
		...
		for idx := 0; idx < 3 && idx < len(p.token); idx++ {
			...
			// 英文匹配
			if matched, _ := regexp2.MustCompile(`[a-zA-Z]{3,}`, regexp2.None).MatchString(token); matched {
				tokenPriority[idx] += 2
			}

			// 日文匹配
			jpMatches := findAllString(JapanesePattern, token)
			if len(jpMatches) >= 2 {
				tokenPriority[idx] += len(jpMatches) * 2
			}

			// 中文匹配
			cnMatches := findAllString(ChinesePattern, token)
			if len(cnMatches) >= 2 {
				tokenPriority[idx] += len(cnMatches) * 2
			}
		}
	...
	backslashRe := regexp2.MustCompile(`^\\|\\$`, regexp2.None)
	animeTitle, _ = backslashRe.Replace(animeTitle, "", -1, -1)
	...
	if len(split) == 1 {
		underscoreRe := regexp2.MustCompile(`_{1}`, regexp2.None)
		if matched, _ := underscoreRe.MatchString(animeTitle); matched {
	...
	if len(split) == 1 {
		...
		chineseChars := findAllString(ChinesePattern, split[0])
		chineseRatio := 0.0
		if len(split[0]) > 0 {
			chineseRatio = float64(len(chineseChars)) / float64(len(split[0]))
		}
		...
		if chineseRatio <= 0.7 {
			...
			for _, idx := range []int{0, len(splitSpace) - 1} {
				if idx >= 0 && idx < len(splitSpace) {
					chsRe := regexp2.MustCompile(`^[\u4e00-\u9fa5]{2,}`, regexp2.None)
					if matched, _ := chsRe.MatchString(splitSpace[idx]); matched {
						chs := splitSpace[idx]
	...
	for _, token := range split {
		jpRe := regexp2.MustCompile(`[\u0800-\u4e00]{2,}`, regexp2.None)
		cnRe := regexp2.MustCompile(`[\u4e00-\u9fa5]{2,}`, regexp2.None)
		enRe := regexp2.MustCompile(`[a-zA-Z]{3,}`, regexp2.None)

		if matched, _ := jpRe.MatchString(token); matched && nameJp == "" {
			nameJp = strings.TrimSpace(token)
		} else if matched, _ := cnRe.MatchString(token); matched && nameZh == "" {
			nameZh = strings.TrimSpace(token)
		} else if matched, _ := enRe.MatchString(token); matched && nameEn == "" {
...
func findAllString(pattern *regexp2.Regexp, s string) []string {
	result := make([]string, 0)
	match, _ := pattern.FindStringMatch(s)
	for match != nil {
